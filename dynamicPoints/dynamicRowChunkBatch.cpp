...
void *calculate_new_values(void *thread_id)
{
    ...
    int iter, ip, jp, i, j;
    double T_gs, T_prev, thr_err, diff, tpad_max, tpnew_max, batch_count = 1;
    for (iter = 0; iter < max_iter; iter++)
    {
        ...
        while (1)
        {
            ...
            int r = row[iter].fetch_add(4);
            if (r > nx)
                break;

            for (ip = r; ip < min(r + 4, nx + 1); ip++)
...
long long solve_gssor_dynamic_chunk_batch()
{

    int i, j, ip, jp;
    double l2err, arrmax1, arrmax2, err_ref;
...
int main()
{
    ...
    // printf("Inputs are: %d %d %lf %lf %lf %lf %d %d\n", nx, ny, xst, xen, yst, yen, num_threads, batch_size);
    ...
    tol = 1.0e-10;
    relax_T = 1.0;
    long long timeTaken = solve_gssor_dynamic_chunk_batch();
