...
int num_threads = 4;
atomic_int count[7] = {num_threads, num_threads, num_threads, num_threads, num_threads, num_threads, num_threads};
...
void solve_gssor(int id,int nx, int ny, double **aP, double **aE, double **aW, double **aN, double **aS, double **b, double **T, double **Tpad, double **Tpnew, int max_iter, double tol, double relax_T)
{

  int i, j, ip, jp, iter;
  double l2err, arrmax1, arrmax2, err_ref, T_gs, rel_err, val;
 ...
 // now perform iterations
  for(iter=0; iter<max_iter; iter++)
  {
    ...
    // for(i=0; i<nx; i++)
    for (i=id; i<nx+2; i+=num_threads)
      for(j=0; j<ny+2; j++)
        {
          val = Tpad[i][j] - Tpnew[i][j];
          l2err += val*val;
        }
    //printf("l2err = %lf\n", l2err);
    l2err = l2err/((double) (nx*ny));
...
int main()
{
  ...
  // ---
    ...
    for (int i = 0; i < num_threads; i++) {
        id = i;
        threads[i] = std::thread(solve_gssor, id, nx, ny, aP, aE, aW, aN, aS, b, T, wrk1, wrk2, max_iter, tol, relax_T);
    }
