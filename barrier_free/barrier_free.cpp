...
atomic<int>* count_arr; // Dimension [max_iter]
atomic<int>* row_number_arr; // Dimension [max_iter]
...
int *thread_ids;
double **thread_errs, **thread_tpad_max, **thread_tpnew_max; // Each of dimension [max_iter, num_threads]
int num_iters_taken;
bool done_calculation = false;

// pthread_barrier_t bar_calc_1;
// pthread_barrier_t bar_calc_2;
...
void *calculate_new_values(void* thread_id) {
    ...
    for (iter = 0; iter < max_iter; iter++) {
        ...
        while (!done_calculation) {
            ip = row_number_arr[iter].fetch_add(1);

            if (ip >= nx) {
                break;
            }

            // Update values of ip row
            for (jp = 1; jp <= ny; jp++) {
                i = ip-1; j = jp-1;
        ...
        if (get_count_iter == num_threads - 1) {
            ...
            arrmax1 = get_max_1d_array(num_threads, thread_tpad_max[iter]);
            arrmax2 = get_max_1d_array(num_threads, thread_tpnew_max[iter]);
...
int main()
{
    ...
    // pthread_barrier_init(&bar_calc_2, NULL, num_threads + 1);
    ...
    count_arr = (atomic<int> *)malloc(max_iter * sizeof(atomic<int>));

    for (i = 0; i < max_iter; i++) {
        count_arr[i].store(0);
    }

    row_number_arr = (atomic<int> *)malloc(max_iter * sizeof(atomic<int>));

    for (i = 0; i < max_iter; i++) {
        row_number_arr[i].store(1);
    }
    ...
    thread_errs = (double **)malloc(max_iter * sizeof(double *));
    for (i = 0; i < max_iter; i++) {
        thread_errs[i] = (double *)malloc(num_threads * sizeof(double));
    }

    thread_tpad_max = (double **)malloc(max_iter * sizeof(double *));
    for (i = 0; i < max_iter; i++) {
        thread_tpad_max[i] = (double *)malloc(num_threads * sizeof(double));
    }

    thread_tpnew_max = (double **)malloc(max_iter * sizeof(double *));
    for (i = 0; i < max_iter; i++) {
        thread_tpnew_max[i] = (double *)malloc(num_threads * sizeof(double));
    }
    ...
    for(i=0; i<max_iter; i++)     free(thread_errs[i]);     free(thread_errs);
    for(i=0; i<max_iter; i++)     free(thread_tpad_max[i]);     free(thread_tpad_max);
    for(i=0; i<max_iter; i++)     free(thread_tpnew_max[i]);     free(thread_tpnew_max);
    free(count_arr);
    free(row_number_arr);
